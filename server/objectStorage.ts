...
export class ObjectStorageService {
  ...
  // Gets the upload URL for provider document uploads
  ...
  async getProviderDocumentFile(documentPath: string): Promise<File> {
    ...
    const documentPath_full = `${documentDir}provider-documents/${documentId}`;
    const { bucketName, objectName } = parseObjectPath(documentPath_full);
    const bucket = objectStorageClient.bucket(bucketName);
  ...
  normalizeProviderDocumentPath(rawPath: string): string {
    ...
    const rawDocumentPath = url.pathname;
    ...
    if (!rawDocumentPath.startsWith(`${documentDir}provider-documents/`)) {
      return rawDocumentPath;
    }
  
    // Extract the document ID from the path
    const documentId = rawDocumentPath.slice(`${documentDir}provider-documents/`.length);
